...
export function CategoryCard({ id, name, description, level, progress, isLocked }: CategoryCardProps) {
    ...
    return (
        <div
            className={`flex flex-col justify-between rounded-2xl border border-slate-200/80 bg-white p-5 shadow-sm transition-all hover:shadow-md ${topBorderColor} ${
        ...
        >
            <div className="space-y-3">
                ...
                <div>
                    <h3 className="text-base font-bold text-slate-900 leading-snug flex items-center gap-1.5">
                        {isLocked && (
                            <svg className="h-4 w-4 text-slate-400 inline" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 15v2m-6 4h12a2 2 0 002-2v-6a2 2 0 00-2-2H6a2 2 0 00-2 2v6a2 2 0 002 2zm10-10V7a4 4 0 00-8 0v4h8z" />
                            </svg>
                        )}
                        {name}
                ...
                {/* Mesaj de atenționare / ghidare */}
                {isInProgress && (
                    <div className="rounded-xl border border-amber-100 bg-amber-50/70 p-2.5 text-xs text-amber-900 leading-relaxed font-medium">
                        Scorul este sub pragul de 90% necesar pentru a debloca nivelul următor.
                    </div>
                )}

                {isUnattempted && (
                    <div className="rounded-xl border border-indigo-100 bg-indigo-50/70 p-2.5 text-xs text-indigo-900 leading-relaxed font-medium">
            ...
            <div className="mt-5 space-y-3.5">
                {isCompleted && (
                    <div className="space-y-1.5">
                        <div className="flex justify-between text-xs font-semibold text-slate-500">
                            <span>Scor</span>
                            <span className={`font-bold ${isMastered ? 'text-emerald-600' : 'text-slate-800'}`}>
                                {score}%
                            </span>
                        </div>
                ...
                {/* Butoane */}
                {isLocked ? (
                    <div className="pt-1.5 text-center text-xs font-medium text-slate-400">
                        Se deblochează la 90% pe toate categoriile de Junior
                    </div>
                ) : isMastered ? (
                    <Link
                        href={`/assessment/${id}`}
                        className="inline-flex w-full items-center justify-center rounded-xl border border-slate-200 bg-white px-4 py-2.5 text-xs font-semibold text-slate-700 shadow-sm transition hover:bg-slate-50"
                    >
                    ...
                    <Link
                        href={`/assessment/${id}`}
                        className="inline-flex w-full items-center justify-center gap-1.5 rounded-xl border border-slate-200 bg-white px-4 py-2.5 text-xs font-semibold text-slate-800 shadow-sm transition hover:bg-slate-50"
                    >
                    ...
                    <Link
                        href={`/assessment/${id}`}
                        className="inline-flex w-full items-center justify-center rounded-xl bg-indigo-600 px-4 py-2.5 text-xs font-semibold text-white shadow transition hover:bg-indigo-700"
                    >
                        Începe Evaluarea
                    </Link>
                )}
